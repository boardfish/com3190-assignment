...
simple_test() ->
    ?assertEqual(enigma:listFor(rotor, "I"),enigma:rotorI()),
    ?assertEqual(enigma:listFor(rotor, "II"),enigma:rotorII()),
    ?assertEqual(enigma:listFor(rotor, "III"),enigma:rotorIII()),
    ?assertEqual(enigma:listFor(reflector, "B"),enigma:reflectorB()).
...
barbarossa_test() ->
    ...
    ?assertEqual("AUFKL XABTE ILUNG XVONX KURTI NOWAX KURTI NOWAX NORDW ESTLX SEBEZ XSEBE ZXUAF FLIEG ERSTR ASZER IQTUN GXDUB ROWKI XDUBR OWKIX OPOTS CHKAX OPOTS CHKAX UMXEI NSAQT DREIN ULLXU HRANG ETRET ENXAN GRIFF XINFX RGTX",Res,{$L,$S,$D}),
    Res2 = enigma:crypt(Enigma,"SFBWD NJUSE GQOBH KRTAR EEZMW KPPRB XOHDR OEQGB BGTQV PGVKB VVGBI MHUSZ YDAJQ IROAX SSSNR EHYGG RPISE ZBOVM QIEMM ZCYSG QDGRE RVBIL EKXYQ IRGIR QNRDN VRXCY YTNJR"),
...
scharnhorst_test() ->
    ...
    Res = enigma:crypt(Enigma,"YKAE NZAP MSCH ZBFO CUVM RMDP YCOF HADZ IZME FXTH FLOL PZLF GGBO TGOX GRET DWTJ IQHL MXVJ WKZU ASTR"),
